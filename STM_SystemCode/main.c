...
#include <string.h>
#include <ctype.h>
//#include "coretick.h"					//we use dwt cycle counter for time base - used for execution times
...
// the code below is used to determine the execution times of each algorithm
...
int main(void)
{
  ...
  char key[50] = "1234";
  char key2[50] = "1235";
  //char compressedString[(sizeOf(original))] = "";
...
while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  unsigned compressedSize = (int)((float)(sizeof(buffer))*(float)(6/(float)8));
	  unsigned char compressedBytes[compressedSize];

	  	  HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_8);
	  	  icm20948_gyro_read_dps(&my_gyro);
	 	  icm20948_accel_read_g(&my_accel);
	 	  sprintf(buffer,"%.*f", FLT_DECIMAL_DIG-1, my_gyro.x, my_gyro.y, my_gyro.z, my_accel.x, my_accel.y, my_accel.z);
		  debugPrintln(&huart2, "Data-Point: ");
	 	  debugPrintln(&huart2, buffer);		//outputs sensor values

//	 	  uint_32 t1 = coreticks();			//starts execution time
		  Compression(compressedBytes, buffer);		//compresses data
//		  t1 = coreticks()-1;				//gets final execution time

		  debugPrintln(&huart2, "Compressed: ");
		  debugPrintln(&huart2, compressedBytes);			//sends compressed data via uart



		  compressedBytes[strcspn(compressedBytes, "\n")] = 0;
		  ...
		  char encryptedMessage[strlen(compressedBytes) + 1];
		  memset(encryptedMessage, '\0', sizeof(encryptedMessage));
//	 	  uint_32 t1 = coreticks();			//starts execution time
		  encryptDecrypt(compressedBytes, encryptedMessage, key);
//		  t1 = coreticks()-1;				//gets final execution time
		  debugPrintln(&huart2, "Encrypted: ");
		  debugPrintln(&huart2, encryptedMessage);

		  char decryptedMessage[strlen(compressedBytes) + 1];
		  memset(decryptedMessage, '\0', sizeof(decryptedMessage));
//	 	  uint_32 t1 = coreticks();			//starts execution time
		  encryptDecrypt(encryptedMessage, decryptedMessage, key2);
//		  t1 = coreticks()-1;				//gets final execution time
		  debugPrintln(&huart2, "Decrypted with wrong key: ");
		  debugPrintln(&huart2, decryptedMessage);

//	 	  uint_32 t1 = coreticks();			//starts execution time
		  Decompression(buff2,  decryptedMessage , compressedSize);
//		  t1 = coreticks()-1;				//gets final execution time
		  debugPrintln(&huart2, "Decompressed: ");
		  debugPrintln(&huart2, buff2);

//	 	  uint_32 t1 = coreticks();			//starts execution time
		  encryptDecrypt(encryptedMessage, decryptedMessage, key);
//		  t1 = coreticks()-1;				//gets final execution time
		  debugPrintln(&huart2, "Decrypted with correct key: ");
		  debugPrintln(&huart2, decryptedMessage);
